...
void OverviewView::mousePressEvent(QMouseEvent* e) {
    if (scene_->itemAt(mapToScene(e->pos()).x(), mapToScene(e->pos()).y(),transform())
            && scene_->itemAt(mapToScene(e->pos()).x(), mapToScene(e->pos()).y(), transform())->boundingRect() == highlightBar_->boundingRect()) {
        barMovement_ = true;
