...
//FIXME: endianness conversion seems to be broken, so we perform it in PVMVolumeReader. joerg
...
void writebits(FILE *file, unsigned int value, int bits) {
    ...
    if (DDS_bufsize + bits < 32) {
        ...
        if (fwrite(&DDS_buffer, 4, 1, file) != 1)
            ERRORMSG();
...
void flushbits(FILE *file) {
    if (DDS_bufsize > 0) {
        ...
        if (DDS_ISINTEL)
            DDS_swapuint(&DDS_buffer);
        if (fwrite(&DDS_buffer, (DDS_bufsize + 7) / 8, 1, file) != 1)
            ERRORMSG();
...
void writeDDSfile(char *filename, unsigned char *data, unsigned int bytes, unsigned int skip, unsigned int strip, int nofree) {
    ...
    if (bytes > DDS_INTERLEAVE)
        version = 2;

    if (skip < 1 || skip > 4)
        skip = 1;
    if (strip < 1 || strip > 65536)
        strip = 1;
    ...
    while (cnt++ < bytes) {
        ...
        if (cnt <= strip)
            ...
            act1 = tmp1 - pre1 - *(ptr1 - strip) + *(ptr1 - strip - 1);
        pre1 = tmp1;
        ...
        if (cnt1 + cnt2 < (1 << DDS_RL) && (cnt1 + cnt2)*max(bits1, bits2) < cnt1*bits1 + cnt2*bits2 + DDS_RL + 3) {
            ...
            while (cnt2-- > 0) {
                ...
                if (ptr2 - strip <= data)
                    ...
                    act2 = tmp2 - pre2 - *(ptr2 - strip) + *(ptr2 - strip - 1);
                pre2 = tmp2;
    ...
    if (nofree == 0)
        ...
        interleave(data, bytes, skip, DDS_INTERLEAVE);
}
...
unsigned char *readDDSfile(char *filename,  unsigned int *bytes) {
    ...
    if ((DDS_file = fopen(filename, "rb")) == NULL)
        return (NULL);
    ...
    while ((cnt1 = readbits(DDS_file, DDS_RL)) != 0) {
        ...
        for (cnt2 = 0; cnt2 < cnt1; cnt2++) {
            if (cnt <= strip)
                ...
                act += *(ptr - strip) - *(ptr - strip - 1) + readbits(DDS_file, bits) - (1 << bits) / 2;
...
void writeRAWfile(char *filename, unsigned char *data, unsigned int bytes, int nofree) {
    ...
    if ((DDS_file = fopen(filename, "wb")) == NULL)
        ERRORMSG();
    if (fwrite(data, 1, bytes, DDS_file) != bytes)
        ERRORMSG();
...
unsigned char *readRAWfile(char *filename, unsigned int *bytes) {
    ...
    do {
        ...
        blkcnt = static_cast<unsigned int>(fread(&data[cnt], 1, DDS_BLOCKSIZE, DDS_file));
        cnt += blkcnt;
...
void writePNMimage(char *filename, unsigned char *image, unsigned int width, unsigned int height, unsigned int components, int dds) {
    ...
    switch (components) {
    case 1:
        snprintf(str, DDS_MAXSTR, "P5\n%d %d\n255\n", width, height);
        break;
    case 2:
        snprintf(str, DDS_MAXSTR, "P5\n%d %d\n32767\n", width, height);
        break;
    case 3:
        snprintf(str, DDS_MAXSTR, "P6\n%d %d\n255\n", width, height);
        break;
    default:
        ERRORMSG();
    }
    ...
    if (DDS_ISINTEL)
        if (components == 2)
            swapshort(data + strlen(str), width*height);

    if (dds != 0)
        writeDDSfile(filename, data, static_cast<unsigned int>(strlen(str)) + width*height*components, components, width);
    else
        writeRAWfile(filename, data, static_cast<unsigned int>(strlen(str)) + width*height*components);
}
...
unsigned char *readPNMimage(char *filename, unsigned int *width, unsigned int *height, unsigned int *components) {
    ...
    if (pnmtype == 5 && maxval == 255)
        *components = 1;
    else if (pnmtype == 5 && (maxval == 32767 || maxval == 65535))
        *components = 2;
    else if (pnmtype == 6 && maxval == 255)
        *components = 3;
    else
        ERRORMSG();

    if ((image = (unsigned char *)malloc((*width) * (*height) * (*components))) == NULL)
        ERRORMSG();
    if (data + bytes != ptr2 + (*width)*(*height)*(*components))
        ERRORMSG();

    if (DDS_ISINTEL)
        if (*components == 2)
            swapshort(ptr2, (*width)*(*height));
...
void writePVMvolume(char *filename, unsigned char *volume,
    ...
    if (width < 1 || height < 1 || depth < 1 || components < 1)
        ERRORMSG();
    ...
    if (description == NULL && courtesy == NULL && parameter == NULL && comment == NULL) {
        if ((data = (unsigned char *)malloc(strlen(str) + width * height * depth * components)) == NULL)
            ERRORMSG();

        memcpy(data, str, strlen(str));
        memcpy(data + strlen(str), volume, width*height*depth*components);

        writeDDSfile(filename, data, static_cast<unsigned int>(strlen(str)) + width*height*depth*components, components, width);
    } else {
        if (description != NULL)
            len1 = static_cast<unsigned int>(strlen((char *)description)) + 1;
        if (courtesy != NULL)
            len2 = static_cast<unsigned int>(strlen((char *)courtesy)) + 1;
        if (parameter != NULL)
            len3 = static_cast<unsigned int>(strlen((char *)parameter)) + 1;
        if (comment != NULL)
            len4 = static_cast<unsigned int>(strlen((char *)comment)) + 1;

        if ((data = (unsigned char *)malloc(strlen(str) + width * height * depth * components + len1 + len2 + len3 + len4)) == NULL)
            ERRORMSG();
        ...
        if (parameter == NULL)
            *(data + strlen(str) + width*height*depth*components + len1 + len2) = '\0';
            ...
            memcpy(data + strlen(str) + width*height*depth*components + len1 + len2, parameter, len3);

        if (comment == NULL)
            *(data + strlen(str) + width*height*depth*components + len1 + len2 + len3) = '\0';
        else
            memcpy(data + strlen(str) + width*height*depth*components + len1 + len2 + len3, comment, len4);

        writeDDSfile(filename, data, static_cast<unsigned int>(strlen(str)) + width*height*depth*components + len1 + len2 + len3 + len4, components, width);
    }
}
...
unsigned char *readPVMvolume(char *filename, 
    ...
    ptr = (unsigned char *)strchr((char *)ptr, '\n') + 1;
    if (version == 3)
        len1 = static_cast<unsigned int>(strlen((char *)(ptr + (*width) * (*height) * (*depth) * numc))) + 1;
    if (version == 3)
        len2 = static_cast<unsigned int>(strlen((char *)(ptr + (*width) * (*height) * (*depth) * numc + len1))) + 1;
    if (version == 3)
        len3 = static_cast<unsigned int>(strlen((char *)(ptr + (*width) * (*height) * (*depth) * numc + len1 + len2))) + 1;
    if (version == 3)
        len4 = static_cast<unsigned int>(strlen((char *)(ptr + (*width) * (*height) * (*depth) * numc + len1 + len2 + len3))) + 1;
